local mod	= DBM:NewMod("YoggSaron", "DBM-Ulduar", 4)
local L		= mod:GetLocalizedStrings()

mod:SetRevision(("$Revision: 4339 $"):sub(12, -3))
...
mod:AddBoolOption("YellBrain", true, "announce")
mod:AddBoolOption("YellMalady", true)
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(63830, 63881) then   -- Malady of the Mind (Death Coil) 
		...
		if args:IsPlayer() and self.Options.YellMalady then
			SendChatMessage(L.YellMalady, "SAY")
		end
	...
	elseif args:IsSpellID(63894) then	-- Shadowy Barrier of Yogg-Saron (this is happens when p2 starts)
		...
		specWarnBrainPortalSoon:Schedule(50)
		warnP2:Show()
		if self.Options.ShowSaraHealth then
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args:IsSpellID(63830, 63881) then	-- Malady of the Mind
		self:RemoveIcon(args.destName)
	elseif args:IsSpellID(63802) then	-- Malady of the Mind
		self:RemoveIcon(args.destName)
	end	
end
...
function mod:SPELL_AURA_REMOVED_DOSE(args)
	if args:IsSpellID(63050) and args.destGUID == UnitGUID("player") then
		...
		elseif args.amount == 30 or args.amount == 25 or args.amount == 20 then
			warnSanity:Show(args.amount)
			specWarnSanity:Show(args.amount)
		elseif args.amount == 15 or args.amount == 10 or args.amount == 5 then
			warnSanity:Show(args.amount)
			specWarnSanity:Show(args.amount)
		end
	end
