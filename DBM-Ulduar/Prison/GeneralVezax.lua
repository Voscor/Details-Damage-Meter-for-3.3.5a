local mod	= DBM:NewMod("GeneralVezax", "DBM-Ulduar", 4)
...
local specWarnLifeLeechSoon		= mod:NewSpecialWarning("SpecialWarningLLSoon")
...
local timerHardmode 			= mod:NewTimer(188, "hardmodeSpawn")		
...
function mod:OnCombatStart(delay)
	DBM:FireCustomEvent("DBM_EncounterStart", 755, "General Vezax")
	if (self.Options.SunwellBoost) then
		timerHardmode				= mod:NewTimer(248, "hardmodeSpawn")
	end
	timerEnrage:Start(-delay)
	timerHardmode:Start(-delay)
	timerNextSurgeofDarkness:Start(-delay)
	timerNextMarkOfTheFaceless:Start(20-delay)
	specWarnLifeLeechSoon:Schedule(15-delay)
end

function mod:OnCombatEnd(wipe)
	DBM:FireCustomEvent("DBM_EncounterEnd", 755, "General Vezax", wipe)
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(63276) then	-- Mark of the Faceless
		...
		specWarnLifeLeechSoon:Schedule(35)
