local mod	= DBM:NewMod("XT002", "DBM-Ulduar", 1)
...
function mod:OnCombatStart(delay)
	...
	if (self.Options.SunwellBoost) then
		timerTympanicTantrum = mod:NewBuffActiveTimer(12, 62776)
	else
		timerTympanicTantrum = mod:NewBuffActiveTimer(8, 62776)
	end
	...
	if self.Options.WarningTympanicTantrumIn10Sec then
		specWarnTTIn10Sec:Schedule(49-delay)
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(63849) then  					-- Exposed Heart
		if self.Options.WarningTympanicTantrumIn10Sec then
			specWarnTTIn10Sec:Cancel()
		end
		timerTympanicTantrumCD:Stop()
		timerHeart:Start()
...
function mod:SPELL_AURA_REMOVED(args)
	...
	--added
	elseif args:IsSpellID(63849) then		-- Exposed Heaert
		timerTympanicTantrumCD:Start()
		if self.Options.WarningTympanicTantrumIn10Sec then
			specWarnTTIn10Sec:Schedule(49)
		end
