local mod	= DBM:NewMod("IronCouncil", "DBM-Ulduar", 2)
...
local specWarnFusionPunch 		= mod:NewSpecialWarning("WarningFusionPunched", 3)
...
mod:AddBoolOption("FusionPunched")
...
local specWarnRuneofPower		= mod:NewSpecialWarning("WarningRuneofPower", 3)
mod:AddBoolOption("PowerRuneWarn", true)
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(61903, 63493) then		-- Fusion Punch
		...
		if args:IsPlayer() and self.Options.FusionPunched then
...
function mod:SPELL_AURA_REMOVED(args)
	if args:IsSpellID(61903, 63493) then
		timerFusionPunchActive:Stop()
	end
end
