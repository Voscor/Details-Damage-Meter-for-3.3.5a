local mod   = DBM:NewMod("Freya", "DBM-Ulduar", 3)
...
local timerSimulKill        = mod:NewTimer(10, "TimerSimulKill")
...
local timerIronRootsCD      = mod:NewCDTimer(30, 62439)
...
mod:AddBoolOption("TremorAlarm", true)
...
function mod:OnCombatStart(delay)
	...
	if (self.Options.SunwellBoost) then
		enrage = mod:NewBerserkTimer(480)
	end
    enrage:Start()
    table.wipe(adds)
...
function mod:SPELL_CAST_START(args)
    if args:IsSpellID(62437, 62859) then
        specWarnTremor:Show()
		if self.Options.TremorAlarm then
